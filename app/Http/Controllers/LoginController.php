...
class LoginController extends Controller
{
   ...
   public function userChecker(){
      ...
      if($is_verified != 1){
         return redirect()->route('app_activationCode', ['token' => $activation_token])
   ...
   public function resendActivationCode($token){
      ...
      $emailSend = new EmailService;
        ...
        return redirect()->route('app_activationCode',
                   ['token' => $token])
   ...
   public function ActivationAccountChangeEmail($token){
      ...
      if($this->request->isMethod('post')){
         ...
         if($user_existe){
            ...
            return back()->with([
               'danger' => 'This address email is already used ! please entrer another email address !',
               'new_email' => $new_email
            ]);
